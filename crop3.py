...
# 외부 의존
from dcgen.utils import ImgSegmentation  # 사용자가 이미 쓰는 유틸
...
DIFF_THRESH    = 45            # ImgSegmentation 인자 유지
DIFF_PORTION   = 0.9           # ImgSegmentation 인자 유지
    ...
    while True:
        ...
        if data is not None and len(data) > 0:
            ...
            if any((d.get("level", 1) != 0) for d in data):
                return data
        last = data
        nxt = cur * RETRY_GROWTH
        if nxt <= MAX_VAR_THRESH:
            cur = nxt
            continue
        if cur != MAX_VAR_THRESH:
            cur = MAX_VAR_THRESH
            continue
        # 최종 실패
...
# ------------------------------
# iPad, iPhone, Xiaomi 비교
# ------------------------------

def get_area_ratio_for_resolution(width: int, height: int) -> float:
    # iPad
    if (width, height) == (2360, 1640) or (width, height) == (1640, 2360):
        return 0.20
    # iPhone
    elif (width, height) == (1170, 2532) or (width, height) == (2532, 1170):
        return 0.40
    # Xiaomi
    elif (width, height) == (1080, 2400) or (width, height) == (2400, 1080):
        return 0.40
    # 그 외
    else:
        print(f"[crop3.py] Unknown resolution {width}x{height}, fallback to 0.40 ratio")
        return 0.40
...
def run_segmentation_recursive(
    image_path: str,
    max_depth: int,
    window_size: int,
    ...
    start_id: int = 0,
    var_thresh: int = 120,
    max_area_ratio: float = None,  # None이면 자동 결정
    max_recursion: int = 3
):
    ...
    if max_area_ratio is None:
        max_area_ratio = get_area_ratio_for_resolution(W, H)
    print(f"[crop3.py] Using max_area_ratio={max_area_ratio} for {W}x{H}")
    ...
    # 디버그 출력
    print(f"[seg] final kept count (level1): {running_idx-1}")
    print(f"[seg] oversize count (not returned): {len(dump_seen)}")
