...
EARLY_EXIT = False
EARLY_EXIT_THRE = 0.5  # 1등 attention * thre > 2등 attention이라면 early exit
...
memo = f"pixel resize {S1_RESIZE_MIN_RATIO}~{S1_RESIZE_MAX_RATIO}" #! 특징
...
from gui_actor.inference import inference  # vanilla inference for full image
...
if __name__ == '__main__':
    ...
    for task in TASKS:
        ...
        init_iter_logger(  
            ...
            headers=[  # 순서 그대로 들어감
                "idx", "orig_w", "orig_h", "crop_time", "num_crop", "early_exit", "num_selected_crop", 
        ...
        for j, item in tqdm(enumerate(screenspot_data)):
            ...
            # maxpixel로 stage 1 resize
            s1_re_ratio = min((S1_RESIZE_MAX_PIXEL / (orig_w * orig_h)) ** 0.5, S1_RESIZE_MAX_RATIO)  #! 최대 S1_RESIZE_MAX_RATIO 까지만 resize (더이상 하면 vanilla보다 오버헤드 큼)
            print(f"🎯 s1 resize ratio : {s1_re_ratio}")
            s0_crop_list = resize_crop_list(crop_list=crop_list, ratio=s1_re_ratio)
            seg_end = time.time()
        ...
        metrics = {
            ...
            "hyperparameters": {
                ...
                "s1_resize_min_ratio": S1_RESIZE_MIN_RATIO,
                "s1_resize_max_ratio": S1_RESIZE_MAX_RATIO,
