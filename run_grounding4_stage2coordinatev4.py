...
SAVE_DIR = "./output/" + "0821_v4_scaled_thumbnail_prompt"
...
#! Hyperparameter =================
VAR_THRESH = 120
WINDOW_SIZE = 120  #<-- 누락되었던 하이퍼파라미터 추가
TOP_Q = 0.5
...
if __name__ == '__main__':
    ...
    s2_question_template = """You are an AI assistant. You will see a small thumbnail of a full screen and some zoomed-in crops.
...
Based on all provided images, identify the precise (x, y) coordinate on the **original screen** for the instruction. Your answer must be only in the format [x, y].
...
"""
    ...
    for task in TASKS:
        ...
        for item in tqdm(screenspot_data, desc=f"Processing {task}"):
            ...
            # 
            # ## 🚨 여기가 에러가 발생했던 부분입니다. 누락된 인자를 모두 추가했습니다.
            # 
            crop_list = run_segmentation_recursive(
                image_path=img_path,
                max_depth=1,
                var_thresh=VAR_THRESH,
                window_size=WINDOW_SIZE,  #<-- 누락된 인자 1
                output_json_path=str(segmentation_dir / "output.json"), #<-- 누락된 인자 2
                output_image_path=str(segmentation_dir),
                start_id=0 #<-- 이전 버전에 있던 인자 추가
            )
        ...
        metrics = {"task": task, "total_samples": len(s2_scores), "accuracy": final_accuracy, "approach": "V4 - Scaled Prompt"}
