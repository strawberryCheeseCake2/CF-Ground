...
#! ================================================================================================
# VisionZip Evaluation Main Function
#! ================================================================================================
...
if __name__ == '__main__':
    ...
    RESIZE_RATIO = args.r
    MAX_CROPS = 3
    REGION_THRESHOLD = args.th
    MIN_PATCHES = 1
    BBOX_PADDING = args.p
    STAGE1_ENSEMBLE_RATIO = args.e
    STAGE2_ENSEMBLE_RATIO = 1 - STAGE1_ENSEMBLE_RATIO
    MAX_PIXELS = 3211264
    
    model_name = "qwen2.5-vl-3B+VisionZip"
    experiment = "visionzip_eval"
    ...
    # VisionZip specific functions
    def visionzip_inference_single_image(conversation, model, processor, topk=1):
        """VisionZip을 사용한 단일 이미지 추론"""
        text = processor.apply_chat_template(conversation, tokenize=False, add_generation_prompt=True)
        image_inputs, video_inputs = process_vision_info(conversation)
        inputs = processor(text=[text], images=image_inputs, videos=video_inputs,
                          padding=True, return_tensors="pt").to(model.device)

        # VisionZip이 적용된 forward pass
        with torch.no_grad():
            out = model(**inputs, output_attentions=True)
        
        # 간단히 마지막 토큰의 어텐션을 사용 (실제로는 더 정교한 처리 필요)
        if hasattr(out, 'attentions') and out.attentions:
            # 마지막 레이어의 어텐션 가져오기
            last_attention = out.attentions[-1]  # (B, H, Q, K)
            # 평균 어텐션 계산
            avg_attention = last_attention.mean(dim=(0, 1)).cpu().numpy()  # (Q, K)
            
            # 비전 토큰에 대한 어텐션만 추출 (실제 구현에서는 토큰 인덱스를 정확히 파악해야 함)
            # 여기서는 간단히 처리
            vision_attention = avg_attention[-1, :len(avg_attention)//2]  # 임시로 절반을 비전 토큰으로 가정
            
            # 2D로 reshape (실제 이미지 크기에 맞춰 조정 필요)
            h_size = int(np.sqrt(len(vision_attention)))
            w_size = h_size
            if h_size * w_size <= len(vision_attention):
                attention_map = vision_attention[:h_size*w_size].reshape(h_size, w_size)
            else:
                attention_map = np.pad(vision_attention, (0, h_size*w_size - len(vision_attention))).reshape(h_size, w_size)
            ...
            if k > 0:
                ...
                pts = []
                vals = []
                vmax = float(flat[indices[0]]) if k > 0 else 1.0
                
                for idx in indices:
                    r, c = divmod(idx, w_size)
                    x = (c + 0.5) / w_size
                    y = (r + 0.5) / h_size
                    pts.append((float(x), float(y)))
                    vals.append(float(flat[idx] / (vmax + 1e-8)))
                
                return {
                    "n_width": w_size,
                    "n_height": h_size,
                    "attn_scores": [attention_map.flatten().tolist()],
                    "topk_points": pts,
                    "topk_values": vals,
                }
        ...
        return {
            "n_width": 28,
            "n_height": 28,
            "attn_scores": [np.ones(28*28).tolist()],
            "topk_points": [(0.5, 0.5)],
            "topk_values": [1.0],
        }
    ...
    def run_stage1_attention_inference(original_image, instruction, model, processor, vis_dir=None):
        """Stage 1: VisionZip을 사용한 리사이즈 추론"""
        ...
        pred = visionzip_inference_single_image(conversation, model, processor, topk=1)
    ...
    # Model loading with VisionZip
    device_map = "mps" if args.mac else "auto"
    
    # VisionZip 모델 로드 (기존 Qwen2_5_VLForConditionalGeneration 대신 VisionZip 버전 사용)
    ...
    # CSV headers
    csv_headers = [
        "method", "resize_ratio", "total_samples", "stage1_accuracy", 
        "avg_iou_top1", "avg_stage1_time", "avg_stage1_tflops", "timestamp"
    ]
    ...
    for task in TASKS:
        init_iter_logger(  
            ...
            headers=[
                ...
                "iou_top1", "s1_time", "s1_tflops", "s1_hit", 
                "total_time", "total_tflops", "s1_acc_uptonow",
        ...
        for j, item in tqdm(enumerate(screenspot_data)):
            ...
            if VISUALIZE:
                ...
                inst_dir = s1_dir = None

            # Stage 1: VisionZip Inference
            if TFOPS_PROFILING:
                prof.reset_profile()

            s1_start = time.time()
            
            s1_pred, resized_image = run_stage1_attention_inference(
            ...
            # IoU calculation (simplified for VisionZip)
            iou_top1 = 0.0
            if s1_original_point:
                # Create a small bounding box around the predicted point for IoU calculation
                point_size = 10  # pixels
                pred_bbox = [
                    max(0, s1_original_point[0] - point_size),
                    max(0, s1_original_point[1] - point_size),
                    min(orig_w, s1_original_point[0] + point_size),
                    min(orig_h, s1_original_point[1] + point_size)
                ]
                iou_top1 = calculate_iou(pred_bbox, original_bbox)
        ...
        metrics = {
            ...
            "avg_times": {
                ...
                "total": s1_time_sum / num_action
            },
            "avg_flops_tflops": {
                "stage1": s1_tflops_sum / num_action,
                "total": s1_tflops_sum / num_action
            },
            "hyperparameters": {
                "resize_ratio": RESIZE_RATIO,
                "attn_impl": ATTN_IMPL,
        ...
        csv_file_path = os.path.join(results_csv_path, f"results_{task}_visionzip.csv")
        ...
        print(f"Avg Times: S1 {metrics['avg_times']['stage1']:.3f}s")
        print(f"Avg FLOPs: S1 {metrics['avg_flops_tflops']['stage1']:.2f} TFLOPs")
        print(f"Avg IoU: {metrics['avg_iou_top1']:.4f}")
        print("=" * 60)

    print("\n📊 VisionZip Evaluation Complete!")
    ...
    print(f"Total avg IoU: {avg_iou_top1:.4f}")
    
    # Save overall results
    cumulative_csv_path = os.path.join("../_results", "results_all_visionzip.csv")
    cumulative_csv_row = [
        model_name,
        RESIZE_RATIO,
        total_samples,
