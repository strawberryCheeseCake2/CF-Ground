"""
qwen25vl.py 전용 시각화 유틸리티
기존 visualize_util.py를 기반으로 하되, qwen25vl.py의 데이터 구조에 맞게 조정
"""
...
def qwen25vl_visualize_stage1_attention_crops(s1_pred, resized_image, crop_list, original_image, save_dir, instruction, gt_bbox=None, s1_predicted_point=None):
    """
    qwen25vl.py용 Stage 1 attention 맵과 생성된 crop들을 시각화
    
    Args:
        s1_pred: qwen_attn_inference 결과 dict
            - 'attn_scores': [[attn_scores_1d]]
            - 'n_width': int
            - 'n_height': int
            - 'resize_ratio': float
        resized_image: PIL Image (리사이즈된 이미지)
        crop_list: crop 정보 리스트 [{'img': PIL Image, 'bbox': [l,t,r,b], 'id': int, 'score': float}, ...]
        original_image: PIL Image (원본 이미지)
        save_dir: str (저장 디렉토리)
        instruction: str
        gt_bbox: [left, top, right, bottom] (옵션)
        s1_predicted_point: [x, y] (원본 이미지 좌표, 옵션)
    """
    
    # 1. Attention 맵 생성
    if 'attn_scores' in s1_pred and s1_pred['attn_scores']:
        attn_scores = s1_pred['attn_scores'][0]  # 첫 번째 이미지의 어텐션 스코어
        ...
        print(f"🌄 Stage 1 visualization saved: {save_path}")
    
    else:
        print("⚠️ No attention scores found for visualization")


def qwen25vl_visualize_stage2_multi_attention(s2_pred, crop_list, original_image, save_dir, instruction, predicted_point=None):
    """
    qwen25vl.py용 Stage 2 multi-image inference 결과 시각화
    
    Args:
        s2_pred: qwen_attn_multi_image_inference 결과 dict
            - 'per_image': [
                {
                    'index': int,
                    'topk_points': [[x, y], ...],  # 정규화된 좌표
                    'topk_values': [score, ...]
                }, ...
              ]
        crop_list: crop 정보 리스트
        original_image: PIL Image
        save_dir: str
        instruction: str
        predicted_point: [x, y] (Stage2 최고 점수 포인트, 원본 좌표)
    """
    ...
    for img_idx, img_result in enumerate(per_image_results):
        ...
        # 원본 crop 이미지 표시 (어텐션 맵 없이)
        plt.imshow(crop_img)
        
        # 예측 점들 표시 (crop 내 좌표)
        crop_w, crop_h = crop_img.size
        for i, (point, score) in enumerate(zip(topk_points[:5], topk_values[:5])):  # 상위 5개 표시
            # 정규화된 좌표를 픽셀 좌표로 변환
            ...
            sizes = [120, 100, 80, 60, 40]
            color = colors[i] if i < len(colors) else 'gray'
            size = sizes[i] if i < len(sizes) else 20
            
            plt.scatter(pixel_x, pixel_y, c=color, s=size, marker='*', 
                       edgecolors='white', linewidth=2, alpha=0.9)
            
            # 점수 표시
            plt.text(pixel_x + 10, pixel_y - 10, f'{i+1}:{score:.3f}', 
                    color='white', fontsize=8, weight='bold',
                    bbox=dict(boxstyle="round,pad=0.3", facecolor='black', alpha=0.7))
        
        plt.title(f'Crop {img_idx+1} (ID: {crop_info.get("id", img_idx+1)})', fontsize=10)
        plt.axis('off')
    ...
    print(f"🌄 Stage2 multi-image visualization saved: {individual_save_path}")


def qwen25vl_visualize_stage3_point_ensemble(s3_ensemble_candidates, original_image, crop_list, original_bbox, 
                                              s3_ensemble_point, s2_corrected_point, s1_original_point,
                                              stage1_ratio, stage2_ratio, save_dir, vis_only_wrong=False, 
                                              stage3_success=True):
    """
    qwen25vl.py용 Stage 3 포인트 기반 앙상블 시각화
    
    Args:
        s3_ensemble_candidates: 앙상블 후보들 리스트
            [{'point': [x, y], 'score': float, 's1_score': float, 's2_score': float, 
              'crop_id': int, 'rank_in_crop': int, 's2_rank': int}, ...]
        original_image: PIL Image
        crop_list: crop 정보 리스트
        original_bbox: [left, top, right, bottom]
        s3_ensemble_point: [x, y] (최종 앙상블 포인트)
        s2_corrected_point: [x, y] (Stage2 최고 점수 포인트)
        s1_original_point: [x, y] (Stage1 예측 포인트)
        stage1_ratio: float (Stage1 앙상블 비율)
        stage2_ratio: float (Stage2 앙상블 비율)
        save_dir: str
        vis_only_wrong: bool (실패한 경우만 시각화할지 여부)
        stage3_success: bool (Stage3 성공 여부)
    """
