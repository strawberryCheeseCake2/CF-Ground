...
def inference(conversation, model, tokenizer, data_processor, 
    logits_processor=None, use_placeholder=False, 
    topk=5, fastv_r=0.5):
    """
    conversation = [
        {
    ...
    # vision start
    vision_start_id = data_processor.tokenizer.convert_tokens_to_ids("<|vision_start|>")

    # 3. 토큰 시퀀스에서 해당 ID 의 첫 등장 위치 찾기
    vision_start_index = (input_ids == vision_start_id).nonzero(as_tuple=True)[0][0].item() + 1
    # print(vision_start_index)
    ...
    fastv_config = {
        ...
        "fastv_k": 3,
        "fastv_r": fastv_r,
        "system_token_length": vision_start_index, # same as "image_token_start_index"
        "image_token_length": image_tokens
    }



    generation_config = GenerationConfig(fastv_config=fastv_config)
    # print(generation_config)
    # Override AGG_LAYER-1 to use eager attention
    # Token Count End #########

    config = model.config
    device = model.device

    AGG_LAYER = fastv_config["fastv_k"] # FastV 설정값 예시
    total_layers = config.num_hidden_layers

    # AGG_LAYER 이전 레이어들을 제외한 나머지 레이어 목록
    # sdpa_layer_indices = [i for i in range(total_layers) if i != AGG_LAYER - 1]

    # 모델의 디코더 레이어에 접근하여 모듈 교체
    # for layer_idx, layer in enumerate(model.model.layers):
    #     if layer_idx == AGG_LAYER - 1:
    #         layer.self_attn = Qwen2_5_VLAttention(
    #             config=config,
    #             layer_idx=layer_idx
    #         ).to(device=device, dtype=model.dtype) # 반드시 디바이스와 데이터 타입을 맞춰줍니다.
      
            
            # print(f"Layer {layer_idx}: self_attn 모듈을 eager으로 교체 완료.")
    ...
    keep_indexs = model.model.pruned_indices
    # print(keep_indexs)
    print(keep_indexs.shape[0])

    if keep_indexs is not None:
        # 프루닝이 일어났다면, 원본 input_ids도 프루닝하여 마스크를 생성합니다.
        pruned_input_ids = inputs["input_ids"][0][keep_indexs]
        pointer_pad_mask = (pruned_input_ids == model.config.pointer_pad_token_id)
    else:
        # 프루닝이 일어나지 않았다면 (예: AGG_LAYER 이전), 기존 방식을 사용합니다.
