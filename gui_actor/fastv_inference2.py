...
def inference(conversation, model, tokenizer, data_processor, 
    ...
    # 모델에 fastv_config를 임시로 설정
    original_fastv_config = getattr(model, '_fastv_config', None)
    model._fastv_config = fastv_config
    
    results = model.generate(**generate_kwargs)
    
    # fastv_config 복원
    if original_fastv_config is not None:
        model._fastv_config = original_fastv_config
    else:
        delattr(model, '_fastv_config')
    ...
    pred["topk_points"] = region_points[:topk] if len(region_points) > topk else region_points
    pred["topk_values"] = region_scores[:topk] if len(region_scores) > topk else region_scores
    pred["topk_points_all"] = region_points_all[:topk] if len(region_points_all) > topk else region_points_all
