...
# multi_image_inference.py (기존 inference.py 기반으로 수정)
...
def inference(conversation, model, tokenizer, data_processor, logits_processor=None, use_placeholder=False, topk=5):
    ...
    # ... (output_text 디코딩은 동일) ...
    generated_ids = results.sequences[0][len(inputs["input_ids"][0]):]
    output_text = tokenizer.decode(generated_ids, skip_special_tokens=False, clean_up_tokenization_spaces=False)
    
    # ... (pointer_pad_mask 확인 로직은 동일) ...
    
    # --- 3. 어텐션 스코어 추출 (기존과 거의 동일) ---
    if use_placeholder:
    ...
    for i in range(num_images):
        _, h, w = (image_grid_thw[i] // model.visual.spatial_merge_size).tolist()
        patch_counts.append(w * h)
        patch_dims.append({'n_width': w, 'n_height': h})

    # 전체 어텐션 스코어를 각 이미지의 패치 수에 맞게 분할합니다.
    # attn_scores의 shape은 (num_targets, total_num_patches) 입니다. 여기서는 num_targets=1.
    per_image_attn_scores = torch.split(attn_scores[0], patch_counts)

    # 각 이미지에 대해 좌표를 계산합니다.
    per_image_results = []
    for i in range(num_images):
        img_attn_scores = per_image_attn_scores[i].unsqueeze(0) # (1, num_patches_for_this_image)
        dims = patch_dims[i]
        n_width, n_height = dims['n_width'], dims['n_height']
        
        best_point, region_points, region_scores, region_points_all = get_prediction_region_point(
            img_attn_scores, n_width, n_height, return_all_regions=True
        )
        ...
        per_image_results.append({
            ...
            "n_width": n_width,
            "n_height": n_height,
            "attn_scores": img_attn_scores.tolist(),
            "topk_points": topk_points,
            "topk_values": topk_values,
            "topk_points_all": topk_points_all,
        })
